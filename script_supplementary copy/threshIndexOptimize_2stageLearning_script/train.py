...
from MAXL_task_weight_script.model import MAXL_ST_estimater as MAXL_task_weight_ST_estimater
...
def train_net(args, model, optimizer, train_loader, val_loader, test_loader, loss_function, metric_func, gene_name_dict):
    ...
    st_estimater.load_state_dict(torch.load(("/media/user/HD-QHAU3/ST_estimation_data/result/img/%s/5-fold/w_pretrain_lr=3e-5/train_all_gene/resnet_ST_net_loss=pcc/model/fold=%d_seed=%d-best_model.pkl" % (args.dataset, args.fold, args.seed)) ,map_location=args.device))
    ...
    best_val_pearsonr = metrics["pearsonr"]
    torch.save(st_estimater.state_dict(), ("%s/model/fold=%d_seed=%d-best_st_estimater.pkl") % (args.output_path, args.fold, args.seed))
    logging.info('[PreTrain Best: @ pearsonr: %.7f, ' % (best_val_pearsonr))
    best_epoch = 0
    
    cnt = 0
    meta_flag = 1
    meta_best_pearson = -100000000
    for epoch in range(args.num_epochs):
            ...
            for _ in range(1000):
                ...
                for t_data, v_data in tqdm(zip(cycle(meta_train_loader), meta_val_loader), total=len(meta_train_loader), leave=False):
                    ...
                    with higher.innerloop_ctx(st_estimater_copy, inner_opt, copy_initial_weights=False, track_higher_grads=True) as (fnet, diffopt):
                        ...
                        for k in range(2):
                            ...
                            all_train_loss = (loss_list * loss_weight["w"].unsqueeze(0)).sum(1) /sum(loss_weight["w"].cpu().detach().numpy())
                ...
                with torch.no_grad():
                    for iteration, data in enumerate(tqdm(val_loader, leave=False)):
                        ...
                        loss = sum(loss_prime)/len(eval_gene_idx)
                ...
                logging.info('[best roop: %d, current roop: %d, @ pearsonr: %.7f, @ Thresh-k: %d ' % (best_roop, roop, meta_val_pearson[-1], loss_weight["k"].cpu().detach().numpy().item()))
                roop+=1

                log_dict["task_wgt"].append(loss_weight["thresh"].cpu().detach().numpy().item())
                log_dict["k"].append(loss_weight["k"].cpu().detach().numpy().item())
                make_thresh_graph(args, log_dict["task_wgt"], "%s/task_wgt/fold=%d_seed=%d_epoch=%d_task_wgt_graph.png" % (args.output_path, args.fold, args.seed, epoch))
                make_thresh_graph(args, log_dict["k"], "%s/task_wgt/fold=%d_seed=%d_epoch=%d_thresh_hold_k.png" % (args.output_path, args.fold, args.seed, epoch))
                
                
                if _!=0 and _%5==0:
                    make_task_wgt_line_graph(args, loss_weight["w"].cpu().detach().numpy().reshape(1, -1)[0], all_gene_name, gene_name_dict["train_hvg_Sort"],  "%s/task_wgt/fold=%d_seed=%d_epoch=%d_itr=%d_task_wgt_line_graph.png" % (args.output_path, args.fold, args.seed, epoch, _))
        ...
        if best_val_pearsonr < log_dict["val_pearsonr"][-1]:
            ...
            cnt += 1
            if args.meta_pacience == cnt:
                st_estimater = MAXL_task_weight_ST_estimater(args, len(eval_gene_idx)+len(train_highly_gene_idx)+len(train_low_gene_idx)).to(args.device)
                st_estimater.load_state_dict(torch.load(("%s/model/fold=%d_seed=%d-best_st_estimater.pkl") % (args.output_path, args.fold, args.seed) ,map_location=args.device))
                st_estimater_optimizer = torch.optim.Adam(st_estimater.parameters(), lr=args.lr)
                meta_flag = 1
                cnt=0
            
                if meta_best_pearson < best_val_pearsonr:       
                    meta_best_pearson = best_val_pearsonr
                else:
                    break


        if best_epoch==0:
            logging.info('best epoch: %d , pearsonr: %.4f' %
                    (best_epoch, best_val_pearsonr))
        else:
