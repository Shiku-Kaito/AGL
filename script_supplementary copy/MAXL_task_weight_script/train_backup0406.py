...
def train_net(args, model, optimizer, train_loader, val_loader, test_loader, loss_function, metric_func, gene_name_dict):
    ...
    for epoch in range(args.num_epochs):
        ...
        for iteration, data in enumerate(tqdm(train_data_loader, leave=False)): #enumerate(tqdm(train_loader, leave=False)):
            ...
            loss_prime = task_weighted_pcc_loss(y["y"][:, eval_gene_idx], st[:, eval_gene_idx], loss_weight["w"][eval_gene_idx], np.array(slide_ids))
            loss_aux = task_weighted_pcc_loss(y["y"][:, aux_gene_idx], st[:, aux_gene_idx], loss_weight["w"][aux_gene_idx], np.array(slide_ids))
        ...
        if epoch!=0 and epoch%5==0:
            ...
            while(1):
                gt_st, pred_st, losses, slide_ids_list = [], [], [], []
                for v_data, t_data in tqdm(zip(meta_val_loader, cycle(meta_train_loader)), leave=False):
                    ...
                    for i in range(512):
                        ...
                        loss_aux = task_weighted_pcc_loss(sampled_y[:, aux_gene_idx], sampled_st[:, aux_gene_idx], loss_weight["w"][aux_gene_idx], np.array(slide_ids))
                        train_loss = 1 - ((loss_prime+loss_aux)/y["y"].shape[1])
                    ...
                    loss_prime = task_weighted_pcc_loss(y["y"][:, eval_gene_idx], v_st[:, eval_gene_idx], loss_weight["w"][eval_gene_idx], np.array(slide_ids))
            ...
            make_toy_task_wgt_graph(args, log_dict["task_wgt"], train_highly_gene_idx, train_toy_gene_idx, "%s/task_wgt/fold=%d_seed=%d_task_wgt_graph.png" % (args.output_path, args.fold, args.seed))
        ...
        with torch.no_grad():
            for iteration, data in enumerate(tqdm(val_loader, leave=False)):
                ...
                loss = 1 - (pcc_loss(y["y"][:, eval_gene_idx], st[:, eval_gene_idx], np.array(slide_ids))/len(eval_gene_idx))
