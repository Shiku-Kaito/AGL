...
def load_data(args):  # LIMUC
    ######### load data #######
    if args.train_highly_variable_top_genes!=None:
        all_gene_adata = sc.read('%s/data/preprocessed_data/hest_1slide/%s_%s%d_matching_patch_adata.h5ad' % (args.input_path, args.dataset, args.gene_select_type, args.train_highly_variable_top_genes))
    ...
    target_gene_adata = sc.read('%s/data/preprocessed_data/hest_1slide/%s_top%d_matching_patch_adata.h5ad' % (args.input_path, args.dataset, args.eval_highly_variable_top_genes))
    ...
    train_dataset = Hest_1slide_Dataset(args=args, adata=all_gene_adata, patch=patch, patch_idx=patch_idx_dict["train"], slide_id=args.dataset, is_train=True)
    ...
    val_dataset = Hest_1slide_Dataset(args=args, adata=all_gene_adata, patch=patch, patch_idx=patch_idx_dict["validation"], slide_id=args.dataset, is_train=False)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False,  num_workers=args.num_workers)  
    test_dataset = Hest_1slide_Dataset(args=args, adata=all_gene_adata, patch=patch, patch_idx=patch_idx_dict["test"], slide_id=args.dataset, is_train=False)
...
def hest_1slide_2stage_meta_load(args):  # LIMUC
    ...
    meta_train_dataset = Hest_1slide_Dataset(args=args, adata=all_gene_adata, patch=patch, patch_idx=patch_idx_dict["train"], slide_id=args.dataset, is_train=True)
    ...
    meta_val_dataset = Hest_1slide_Dataset(args=args, adata=all_gene_adata, patch=patch, patch_idx=patch_idx_dict["validation"], slide_id=args.dataset, is_train=True)
