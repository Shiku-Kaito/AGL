...
def train_net(args, 
    ...
    file_handler = logging.FileHandler("%s/log_dict/fold=%d_seed=%d_training_setting.log" 
                                       % (args.output_path, args.fold, args.seed))
    ...
    for epoch in range(args.num_epochs):
        ...
        if best_val_pearsonr < log_dict["val_pearsonr"][-1]:
            ...
            cnt = 0
            best_epoch = epoch
            torch.save(st_estimater.state_dict(), 
                       ("%s/model/fold=%d_seed=%d-best_st_estimater.pkl") % (args.output_path, args.fold, args.seed))
        else:
            cnt += 1
        ...
        Make_CutOff_k_graph(args, log_dict["k"], 
                            "%s/k_value/fold=%d_seed=%d_thresh_hold_k.png" % (args.output_path, args.fold, args.seed))
