...
def train_net(args, model, optimizer, train_loader, val_loader, test_loader, loss_function, metric_func, gene_name_dict):
    ...
    for epoch in range(args.num_epochs):
        ...
        for iteration, data in enumerate(tqdm(train_loader, leave=False)): #enumerate(tqdm(train_loader, leave=False)):
            ...
            loss, log_vars, y = loss_function(patch, st, slide_ids)

            loss.backward()
            optimizer.step()
            optimizer.zero_grad()

            gt_st.extend(st.cpu().detach().numpy()), pred_st.extend(y.cpu().detach().numpy())
        ...
        if best_val_pearsonr < log_dict["val_pearsonr"][-1]:
            ...
            cnt = 0
            best_epoch = epoch
            torch.save(loss_function.state_dict(), ("%s/model/fold=%d_seed=%d-best_model.pkl") % (args.output_path, args.fold, args.seed))
