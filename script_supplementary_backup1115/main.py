...
def main(args):
    ...
    if args.toy_gene != 1:
        if args.train_highly_variable_top_genes != None:
            args.output_path += '%s/%s/%s/w_pretrain_lr=3e-5/train_%s_genes=%d/%s_loss=%s/' % (args.img_or_token_feat_or_feat, args.dataset, args.data_type, args.gene_select_type, args.train_highly_variable_top_genes, args.mode, args.loss) 
        elif args.train_highly_variable_top_genes == None:
            args.output_path += '%s/%s/%s/w_pretrain_lr=3e-5/train_all_gene/%s_loss=%s/' % (args.img_or_token_feat_or_feat, args.dataset, args.data_type, args.mode, args.loss) 
...
if __name__ == '__main__':
    ...
    #    for seed in range(1):
        ...
        # Module Selection
        parser.add_argument('--module',default='resnet_ST_net', 
        ...
        parser.add_argument('--output_path',
                            default='/media/user/HD-QHAU3/ST_estimation_data/result/', type=str, help="output file name")
        # input path
        ...
        parser.add_argument('--weight_estimater_lr', default=3e-3,
                            type=float, help='learning rate')   
        parser.add_argument('--sampling_num', default=320, type=int, help='') 
        ...
        parser.add_argument('--temper', default=0.01, type=float, help='random or top')
